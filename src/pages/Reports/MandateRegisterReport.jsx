...
import { MandateResisterReports } from "../../api/ApiFunction";
import { toast } from "react-toastify";
import { Loader2 } from "lucide-react";
import { useAuth } from "../../context/AuthContext";

const MandateResisterReport = () => {
  // Data type options
  ...
  const formik = useFormik({
    ...
    onSubmit: async ({ fromDate, toDate, dataType, source }) => {
      ...
      try {
        const response = await MandateResisterReports(req);
        if (response.status) {
          setIsLoading(false);
    ...
    <>
      ...
      {reportData === null ? (
        ...
        <JsontoExcel
          ...
          reportType="Mandate-Register-Report"
        />
      )}
    </>
  );
...
export default MandateResisterReport;
